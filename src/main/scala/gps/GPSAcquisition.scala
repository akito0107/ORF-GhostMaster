...
import sample.RemoteCacheContainer
import sample.RemoteCacheContainer
import sample.SampleTaskImpl
import sample.SampleTaskKeys
import sample.SampleUtil
 ...
 */
class GPSAcquisition(_gateway: Gateway) {
  ...
  */
  ...
  val APP_NAME = "gps_acquisition"
  val TASK_NAME = "gps_task"
  ...
  def runApp: Unit = {
    ...
    mTaskCache.put(TASK_ID, new GPSTaskImpl())
    ...
    for (i <- 0 until 1000) {
      ...
      val data: OffloadableData = GPSUtil.genData(TASK_ID, seq)
      data.putData(GPSTaskKeys.DEBUG, null)
      //    data.debugData()
